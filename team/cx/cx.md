...
> 2026-04-28 · PDP 시안 폴더 정리 — `images/pdp_*_trust_ux*.png` 4장을 `images/pdp_v1_suncream/`로 이동, §7-2 이미지 경로 업데이트. v2 베개 시안용 `images/pdp_v2_pillow_gmarket/` 폴더 신설.
> 2026-05-02 · §14 신설 — PDP v2 호텔식 베개 시안(TF2 산출물 ①) 결정사항·Before/After/콜아웃/인서트·데이터 근거 정합 검증 정리.
...
| Before | After |
|:---:|:---:|
| ![PDP Before — 상품 정보와 가격 중심](images/pdp_v1_suncream/pdp_before_trust_ux_2.png) | ![PDP After — 신뢰 UX 적용](images/pdp_v1_suncream/pdp_after_trust_ux_2.png) |
...
## 14. PDP v2 산출물 — 호텔식 베개 시안 (TF2 산출물 ①)

§7-2의 PDP Before/After 가설을 본 시안으로 구현한 결과물이다.
v1(선크림 시안)에서 v2(호텔식 메모리폼 베개 시안)로 전환하면서, Olist 데이터 정합성과 G마켓 통합 PDP 톤을 모두 반영했다.

### 14-1. v1 → v2 전환 배경

| 항목 | v1 (선크림) | v2 (호텔식 베개) | 전환 사유 |
|------|-----------|----------------|----------|
| 카테고리 | `health_beauty` | `bed_bath_table` | Olist 재구매율 1위 카테고리(`6.42%`)로 TF3 침구 크로스셀링과 시각 동선 일관 |
| 시각 톤 | 임의 톤 | G마켓 통합 PDP | Olist의 통합 PDP 템플릿 구조와 정합 (네이버 스마트스토어는 셀러별 자유 디자인이라 부정합) |
| 가격 | 임의 | `27,300원` (≈`85 BRL`) | Olist 침구 가격 중앙값(`79 BRL`) 정합 — `30~100 BRL` 최빈 구간(`58.3%`) |
| 무게 | 미설정 | 약 `700g` | Olist 침구 무게 중앙값(`1,275g`) 근처 — `1~2kg` 최빈 구간(`34.5%`) |
| 가상 셀러 | 미설정 | `홈슬립스튜디오` (Fast-Ship 인증) | TF1 셀러 인증 태깅과 직접 연결 |

> v1 시안(§7-2)은 분석 컨텍스트 보존을 위해 `images/pdp_v1_suncream/`에 그대로 archive 한다.

### 14-2. Before vs After 시안

핵심은 상품 이미지가 아니라, **신뢰 정보 4종(별점·배송·반품·셀러)이 한 화면에 박스로 모이는가**이다.

| Before — 신뢰 정보 분산 | After — 신뢰박스 ①~④ |
|:---:|:---:|
| ![PDP Before — 호텔식 베개 (G마켓 톤)](images/pdp_v2_pillow_gmarket/pdp_before_main.png) | ![PDP After — 호텔식 베개 (G마켓 톤)](images/pdp_v2_pillow_gmarket/pdp_after_main.png) |

#### Before — Olist의 현 상황 반영

신뢰 정보가 *없는* 게 아니라 *흩어져 있다*는 점을 보여준다. 가격 아래에 분산된 회색 텍스트 6줄(분할결제·적립·쿠폰·셀러 한 줄·배송·반품 안내)과 광고 띠가 채워져 있어, 결제 직전 5초에 신뢰가 한 화면에 모이지 않는다.

- 별점 작게 (`★★★☆☆ 3.8`, 리뷰 247) — 신뢰 시그널 약함
- 배송 표기 모호 (`일반배송 2~3일 예정`) — 정확한 도착일 모름
- 셀러 정보는 한 줄 텍스트(`판매자: 홈슬립스튜디오 (4.0)`)로만 표기
- 반품 안내는 *"상세정보 페이지를 참조"*로 미루기

#### After — 신뢰박스 4종으로 그룹핑

가격 바로 아래에 신뢰박스 4종을 박스화하고, 각 박스 좌측에 4px 색띠 + ①~④ 원형 번호를 박아 매핑을 시각화했다.

| # | 박스 | 핵심 메시지 | 데이터 근거 |
|---|------|-----------|------------|
| ① | 별점 (노랑 색띠) | `⭐ 4.7 · 리뷰 1,203개` + 베스트 리뷰 코멘트 | 5점 재구매율 `3.12%`로 가는 첫 단추 |
| ② | 배송 (초록 색띠) | `🚚 오늘 출발 · 11/15 (금) 도착 보장` | 정시 배송 시 재구매율 `3.04%` (지연 `2.51%`) |
| ③ | 반품 (보라 색띠) | `↻ 30일 무료 반품 · 항균 인증서 동봉` | CS 무응답 `19.8%`(저평점 2위) 선제 차단 |
| ④ | 셀러 (보라 색띠) | `🛡 홈슬립스튜디오 ✓ · ⭐ 4.8 · 응답률 99%` | TF1 셀러 인증 태깅과 직접 연결 |

추가로 가격 위에 `⚡ Fast-Ship 인증 셀러 · 당일 발송` 배지를 1회 노출한다 (셀러박스 안 중복은 제거).

> **변수 통제**: 가격·옵션·CTA·상품 이미지·썸네일은 양쪽 동일. 변경 변수는 신뢰 정보 4종의 박스화 여부와 Fast-Ship 배지의 유무.

### 14-3. 어노테이션 콜아웃 ①~④

발표 슬라이드용 보조 자료. 박스 좌측의 ①~④ 원형 번호와 1:1로 매핑된다.

| Before — 빨강 콜아웃 (결핍) | After — 초록 콜아웃 (개선 + 데이터 근거) |
|:---:|:---:|
| ![Before 콜아웃 ①~④](images/pdp_v2_pillow_gmarket/pdp_before_callouts.png) | ![After 콜아웃 ①~④](images/pdp_v2_pillow_gmarket/pdp_after_callouts.png) |

### 14-4. After 인서트 — 보조 신뢰 모듈

메인 시안(상단 1스크린)에는 들어가지 않는 G마켓 PDP 풀 스크롤의 임팩트 영역을 발췌한 보조 시안이다.

![After 인서트 — 1위 트로피 · 평점 4.7 · 리뷰카드 3장](images/pdp_v2_pillow_gmarket/pdp_after_insert_review.png)

- 🏆 압도적 판매량 1위 (트로피 배너, 누적 `2,570,000개`)
- ⭐ 증명된 평점률 `4.7` (조작 없는 찐후기 `70,000건` 이상)
- ★★★★★ 진짜 리뷰 카드 3장

발표 슬라이드에 좁은 우측 컬럼으로 배치하거나, Q&A 방어용 보조 슬라이드로 사용한다.

### 14-5. 발표 슬라이드 배치 (16:9)

```
[Before 메인 시안]   [After 메인 시안]   [After 인서트]
   (분산된 신뢰)        (신뢰박스 ①~④)      (압도적 신뢰 누적)
```

| 시간 | 슬라이드 영역 | 발표 멘트 핵심 |
|------|-----------|-----------|
| 0~5초 | Before | *"지금은 신뢰 정보가 흩어져 있어 결제 직전 망설임이 큽니다"* |
| 5~10초 | After | *"결제 직전 5초에 신뢰 4종이 한 화면에 모이는 구조로 재배치했습니다"* |
| 10~15초 | After 인서트 | *"여기에 누적 판매·평점·리뷰까지 더해지면 결정타가 됩니다"* |

콜아웃 패널은 별도 영역(또는 보조 슬라이드)에 인용한다.

### 14-6. 데이터 근거 정합 검증

Olist `bed_bath_table` 카테고리 주문 `n=11,115` 기준으로 가격·무게가 최빈 구간 안에 들어가는지 검증을 끝냈다.

| 항목 | Olist 분포 | 시안 적용 값 | 정합성 |
|------|-----------|-------------|--------|
| 가격 중앙값 | `79.05 BRL` | `27,300원` (≈`85 BRL`) | ✅ 30~100 BRL 최빈 구간(`58.3%`) |
| 무게 중앙값 | `1,275g` | 약 `700g` | ✅ 1~2kg 최빈 구간(`34.5%`) 근처 |
| 카테고리 재구매율 | `6.42%` (1위) | `bed_bath_table` 채택 | ✅ TF3 침구 크로스셀링과 시각 동선 일관 |

> 이 정합성을 지켜야 발표 신뢰성 유지와 Q&A 방어가 가능하다. 변경 시 데이터 재검증이 필요하다.

### 14-7. 산출물 파일 구성

| 파일 | 용도 | 사이즈 |
|------|------|--------|
| `pdp_before_main.png` | Before 메인 시안 | `720×1600` |
| `pdp_after_main.png` | After 메인 시안 | `720×1600` |
| `pdp_after_insert_review.png` | After 보조 인서트 | `540×1100` |
| `pdp_before_callouts.png` | Before 빨강 콜아웃 ①~④ | `720×fit` |
| `pdp_after_callouts.png` | After 초록 콜아웃 ①~④ | `720×fit` |
| `notes.md` | 산출물 SSOT (수정 내역·결정사항·다음 단계) | - |
| `pdp_v2_pillow.pen` | 펜슬 원본 (편집용) | - |

모두 `images/pdp_v2_pillow_gmarket/` 폴더에 위치한다.

### 14-8. 연결 산출물

본 산출물(TF2 ①)은 다음 3개 산출물과 한 흐름으로 연결된다.

| # | 산출물 | 연결 메시지 |
|---|--------|-----------|
| ① | **PDP Before/After** (본 §14) | 첫 경험 신뢰 확보 |
| ② | CX 48시간 SLA | 저평점 `19.8%` 선제 차단 |
| ③ | Thank-you 와이어프레임 | TF3 D+0 자동화로 연결 |
| ④ | 블프 랜딩 5단 | TF3 시크릿 바우처로 연결 |

상세 정의는 `team/doc/산출물/TF2_산출물_구상안_v1.md` 참조.
